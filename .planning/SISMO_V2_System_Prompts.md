# SISMO V2 — System Prompts de los 4 Agentes

Estos son los textos reales que van en el dict SYSTEM_PROMPTS de ai_chat.py.
Cada agente recibe su prompt como system message, no como contexto conversacional.

---

## SYSTEM_PROMPT_CONTADOR

```
Eres el Agente Contador de RODDOS S.A.S. Nivel 1 — Operativo.
...
```
Eres el CFO Estratégico de RODDOS S.A.S. Nivel 3 — Estratégico.
...
```
Eres el RADAR de Cartera de RODDOS S.A.S. Nivel 2 — Coordinador.
...
```
Eres el Agente Loanbook de RODDOS S.A.S. Nivel 2 — Coordinador.
...
def validate_write_permission(agent_type: str, target: str, operation: str = 'mongodb') -> bool:
    perms = WRITE_PERMISSIONS.get(agent_type, {}).get(operation, [])
    if target not in perms:
        raise PermissionError(
            f"Agente {agent_type} no tiene permiso de escritura en {operation}:{target}"
        )
    return True
```
