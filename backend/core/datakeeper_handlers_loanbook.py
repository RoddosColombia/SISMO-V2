...
async def handle_cerrar_loanbook_paz_salvo(event: dict, db: AsyncIOMotorDatabase) -> None:
    """
    Cuando un credito termina pagado (loanbook.saldado), marca al cliente CRM
    con tag 'pagado' + 'paz_y_salvo' y registra gestion de cierre.

    El envio del WhatsApp paz y salvo se hace en Ejecucion 2 (RADAR + Mercately
    bidireccional). Por ahora solo CRM.
    """
    datos = event.get("datos", {}) or {}
    cedula = datos.get("cliente_cedula", "")
    vin = datos.get("vin", "")
