...
- Cuentas contables (CRÍTICAS — sin estas Alegra rechaza con code 1008):
    Ingreso ventas:       41350501
    Inventario activo:    14350101
    Costo de ventas:      61350501
...
- Cuentas contables OBLIGATORIAS (sin estas Alegra rechaza con code 1008):
    Ingreso ventas:    41350601
    Inventario:        14350102
    Costo de ventas:   61350601
...
def _build_lote_motos_prompt(motos, proveedor_nit, proveedor_nombre, numero_factura, fecha):
    lineas = []
    for m in motos:
        vin    = (m.get("vin") or "").strip().upper()
        motor  = (m.get("motor") or "").strip()
    ...
    lista = "\n".join(lineas) if lineas else "(lote vacio)"
    return (
        f"Estas autenticado en Alegra (https://app.alegra.com) cuenta RODDOS S.A.S. "
        f"Si la sesion esta cerrada, login email \"{ALEGRA_EMAIL}\" / password \"{ALEGRA_UI_PASSWORD}\".\n\n"
        f"OBJETIVO: registrar la llegada de un lote de motos. Dos pasos:\n"
        f"  PASO A - crear un item inventariable INDIVIDUAL por cada moto (referencia=VIN).\n"
        f"  PASO B - crear factura de compra (bill) al proveedor con todas las motos.\n\n"
        f"CONFIGURACION OBLIGATORIA por ITEM moto:\n"
        f"- isInventoriable=true, categoria id=1 (Motos nuevas)\n"
        f"- Reference: VIN exacto en MAYUSCULAS\n"
        f"- Cuentas obligatorias (sin estas Alegra rechaza code 1008):\n"
        f"    Ingreso: 41350501 | Inventario: 14350101 | Costo: 61350501\n"
        f"- Precio sin IVA segun modelo (Raider 6554622 / Sport 4831933)\n"
        f"- IVA 19% (id 4) | Costo unitario indicado | Cantidad inicial 1\n"
        f"- Bodega: la default de motos\n\n"
        f"LISTA DEL LOTE ({len(motos)} motos):\n{lista}\n\n"
        f"IDEMPOTENCIA: antes de crear, busca por reference=VIN. Si existe, NO duplicar; "
        f"marcar en items_omitidos con razon='ya_existia' y reusar id en el bill.\n\n"
        f"PASO B - bill al proveedor:\n"
        f"- Provider id: {proveedor_nit}\n"
        f"- Nombre: {proveedor_nombre or 'Auteco Mobility S.A.S.'}\n"
        f"- Numero factura: {numero_factura}\n"
        f"- Fecha: {fecha}\n"
        f"- Lineas: una por moto con cantidad=1, precio=costo unitario\n"
        f"- Auteco NIT 860024781 o 901249413 = AUTORETENEDOR (no aplicar ReteFuente).\n\n"
        f"ENTREGABLE JSON:\n"
        f"- items_creados: [{{vin, alegra_id, nombre}}]\n"
        f"- items_omitidos: [{{vin, alegra_id, razon}}]\n"
        f"- bill_alegra_id: id NUMERICO del bill (extraer de URL final tipo /bill/12345)\n"
        f"- bill_alegra_url\n"
        f"- errores: [{{vin?, etapa, mensaje}}]\n"
        f"- todo_ok=true SOLO si todas las motos quedaron y bill tiene id numerico.\n\n"
        f"NO inventes IDs. NO uses la cadena 'firecrawl' como id. Si no extraes id numerico, marca error."
    )


async def crear_lote_motos_agente(motos, proveedor_nit, numero_factura, fecha,
                                   proveedor_nombre="Auteco Mobility S.A.S."):
    err = _validar_credenciales_alegra()
    if err:
        logger.error("crear_lote_motos_agente - credenciales: %s", err)
        return {"success": False, "error": err, "stage": "credentials"}
    ...
    if not motos:
        return {"success": False, "error": "Lote vacio.", "stage": "validation"}
    for i, m in enumerate(motos):
        if not (m.get("vin") or "").strip():
            return {"success": False, "error": f"moto[{i}] sin VIN", "stage": "validation"}
        if not (m.get("motor") or "").strip():
    ...
    fc = _get_fc()
    prompt = _build_lote_motos_prompt(motos, proveedor_nit, proveedor_nombre, numero_factura, fecha)
    logger.info("crear_lote_motos_agente.start n=%d proveedor=%s factura=%s",
                len(motos), proveedor_nit, numero_factura)
    ...
    try:
        ...
        logger.exception("crear_lote_motos_agente - fc.agent excepcion")
        return {"success": False, "error": f"Agent error: {type(ex).__name__}: {ex}", "stage": "agent_call"}
...
# ─────────────────────────────────────────────────────────────────────────────
# V2 - Repuestos con bodega separada
# ─────────────────────────────────────────────────────────────────────────────
...
_LOTE_REPUESTOS_SCHEMA: dict = {
    ...
    "properties": {
        ...
        "bodega_repuestos_id":      {"type": "string"},
        "bodega_repuestos_creada":  {"type": "boolean"},
        "items_creados":            {"type": "array", "items": {"type": "object"}},
        "items_omitidos":           {"type": "array", "items": {"type": "object"}},
        "bill_alegra_id":           {"type": "string"},
        "bill_alegra_url":          {"type": "string"},
        "errores":                  {"type": "array", "items": {"type": "object"}},
...
def _build_lote_repuestos_prompt(items, proveedor_nit, proveedor_nombre, numero_factura, fecha):
    lineas = []
    for it in items:
        ref = (it.get("referencia") or it.get("reference") or "").strip()
        nombre = (it.get("nombre") or it.get("name") or "Repuesto sin nombre").strip()
    ...
    lista = "\n".join(lineas) if lineas else "(lista vacia)"
    return (
        f"Estas autenticado en Alegra (https://app.alegra.com) cuenta RODDOS S.A.S. "
        f"Si la sesion esta cerrada, login email \"{ALEGRA_EMAIL}\" / password \"{ALEGRA_UI_PASSWORD}\".\n\n"
        f"CONTEXTO CRITICO: en RODDOS los repuestos deben afectar EXCLUSIVAMENTE las cuentas:\n"
        f"  Ingreso: 41350601 | Inventario: 14350102 | Costo: 61350601\n"
        f"Si los items se crean en bodega 'Motos' (default), Alegra contabiliza con cuentas de motos\n"
        f"y el balance queda mal. Por eso este flujo trabaja con bodega 'Repuestos' separada.\n\n"
        f"PASO 0 - VERIFICAR/CREAR bodega 'Repuestos':\n"
        f"1. Navega a /inventory/warehouses (o /warehouses).\n"
        f"2. Busca bodega cuyo nombre sea exactamente 'Repuestos' (case-insensitive).\n"
        f"3. Si existe: bodega_repuestos_id=su_id, bodega_repuestos_existia=true.\n"
        f"4. Si NO existe: crearla con nombre 'Repuestos', descripcion 'Almacen RODDOS para repuestos',\n"
        f"   por defecto para items categoria Repuestos id=5. bodega_repuestos_creada=true.\n\n"
        f"PASO 1 - CREAR ITEMS (uno por linea):\n"
        f"- isInventoriable=true, categoria id=5 (Repuestos)\n"
        f"- Reference: la referencia exacta del proveedor.\n"
        f"- Bodega/almacen: bodega 'Repuestos' del paso 0.\n"
        f"- Cuentas (sin estas Alegra rechaza code 1008):\n"
        f"    Ingreso: 41350601 | Inventario: 14350102 | Costo: 61350601\n"
        f"- Precio sin IVA: el precio_unit indicado.\n"
        f"- IVA: si iva_pct=19 -> tax id 4. Si 0 -> sin tax.\n"
        f"- Costo unitario: precio_unit. Cantidad inicial: 0 (entra con el bill).\n"
        f"- IDEMPOTENCIA: si existe item con esa reference, no duplicar; items_omitidos razon='ya_existia'.\n\n"
        f"PASO 2 - BILL DEL PROVEEDOR:\n"
        f"- Provider id: {proveedor_nit}\n"
        f"- Nombre: {proveedor_nombre or 'Auteco S.A.S.'}\n"
        f"- Numero factura: {numero_factura}\n"
        f"- Fecha: {fecha}\n"
        f"- Bodega destino: bodega 'Repuestos'.\n"
        f"- Items: uno por cada linea de la lista, con cantidad y precio_unit.\n"
        f"- Auteco NIT 860024781 o 901249413 = AUTORETENEDOR (no aplicar ReteFuente).\n\n"
        f"LISTA DE REPUESTOS ({len(items)} lineas):\n{lista}\n\n"
        f"ENTREGABLE JSON:\n"
        f"- bodega_repuestos_id, bodega_repuestos_existia, bodega_repuestos_creada\n"
        f"- items_creados: [{{referencia, alegra_id, nombre, cantidad}}]\n"
        f"- items_omitidos: [{{referencia, alegra_id, razon}}]\n"
        f"- bill_alegra_id (numerico, de URL final)\n"
        f"- bill_alegra_url\n"
        f"- errores: [{{etapa, referencia?, mensaje}}]\n"
        f"- todo_ok=true SOLO si bodega_repuestos_id existe, todos los items quedaron, bill numerico.\n\n"
        f"NO inventes IDs. Si algo falla, todo_ok=false con causa concreta."
    )


async def crear_lote_repuestos_agente(items, proveedor_nit, numero_factura, fecha,
                                       proveedor_nombre="Auteco S.A.S."):
    err = _validar_credenciales_alegra()
    if err:
        logger.error("crear_lote_repuestos_agente - credenciales: %s", err)
        return {"success": False, "error": err, "stage": "credentials"}
    ...
    try:
        ...
        logger.exception("crear_lote_repuestos_agente - fc.agent excepcion")
        return {"success": False, "error": f"Agent error: {type(ex).__name__}: {ex}", "stage": "agent_call"}
